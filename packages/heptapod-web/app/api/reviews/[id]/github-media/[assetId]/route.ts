...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  const mediaUrl = await resolveGitHubMediaUrl(github, assetId, evidence.kind);
  if (!mediaUrl) return NextResponse.json({ error: "Media source is unavailable" }, { status: 404 });

  const range = request.headers.get("range");
  const response = await fetch(mediaUrl, { headers: range ? { Range: range } : undefined, cache: "no-store" });
  if (!response.ok && response.status !== 206) {
    return NextResponse.json({ error: "Media source could not be loaded" }, { status: 502 });
  }
