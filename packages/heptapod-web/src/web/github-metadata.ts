...
import type { GitHubPullRequestMetadata, GitHubSource } from "@thestraylight/heptapod/types";
...
export async function resolveGitHubVideoUrl(source: GitHubSource, assetId: string): Promise<string | null> {
  ...
  try {
    ...
    const videoSources = [...html.matchAll(/<video[^>]+src="([^"]+)"/gi)].map((match) => match[1].replaceAll("&amp;", "&"));
    return videoSources.find((url) => url.includes(assetId)) ?? null;
  } catch {
    return null;
  }
}
